...
| 이미지 정리 재시도 | 원본·썸네일을 하나의 내부 채팅 이미지 자산으로 묶어 같은 작업 잠금으로 직렬화한다. 삭제 이미지가 활성 참조·신고 snapshot에 쓰이지 않을 때만 `media_cleanup_tasks`에 적재하며, 새 메시지는 업로드 원본 `url`만 사용한다. 정리 완료 URL은 재업로드 전까지 다시 전송할 수 없고, 삭제 실패는 지수 backoff로 재시도한다. |
