...
특이사항:
  ...
  - 기본 닉네임은 가입 진행 중 placeholder이며, 예약어가 아닌 닉네임·학번·학과가 모두 채워져야 프로필 완료로 본다. 프로필 사진은 완료 조건이 아니다.
  - 신규·변경 닉네임은 trim·Unicode NFC 정규화 후 ACTIVE 회원 사이에서만 고유해야 하며, `스쿠리 유저`·`운영자` 포함 닉네임은 중간 공백 우회까지 차단한다. 탈퇴 시 고유 키를 해제해 재사용을 허용하고 기존 중복 닉네임은 임의 변경하지 않는다.
