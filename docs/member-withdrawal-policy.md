...
| Friend (Phase 14 계획) | 공개 프로필 삭제, 활성 친구 코드 영구 폐기, 요청·관계·설정·차단 정리, 처리 중 초대 만료 |
...
### 3.6 Friend (Phase 14 계획)

> 아래 규칙은 승인된 Phase 14 계획이며 현재 런타임에는 아직 구현되지 않았다. 실제 Entity·API·탈퇴 orchestration 변경은 사용자의 별도 코드 구현 승인 후 진행한다.
