...
> 상태: Foundation과 친구 관계 Core(요청·friendship·즐겨찾기·친구 끊기·차단·PENDING 목록), Minecraft 안전 projection, 시간표 공유, TaxiParty·Chat 초대와 초대·정원·파티원 UX 보완, 친구·초대 알림과 Friend derived-data 탈퇴 cleanup의 Backend #88 전달을 완료했다. Frontend #30은 앱 알림 연동 구현·자동 검증·리뷰 대응을 마쳤으며, 병합·배포 뒤 실제 기기 통합 QA를 진행한다.
...
| Notification | FRIEND_REQUEST, FRIEND_ACCEPTED, FRIEND_DECLINED, PARTY_INVITATION, CHAT_ROOM_INVITATION의 최신 상태 검증 인앱·SSE·FCM 전달 |
...
- Notice 권한 예시:
  - 공개 조회:
    ...
    - `GET /v1/app-notices/{appNoticeId}`
  - 인증 사용자 전용:
    - `POST/DELETE /v1/app-notices/{appNoticeId}/like`
    - `GET/POST /v1/app-notices/{appNoticeId}/comments`
    - `POST/DELETE /v1/app-notice-comments/{commentId}/like`
  ...
  - 공지 댓글 작성자 전용:
    ...
    - 위반 시 `403 NOT_APP_NOTICE_COMMENT_AUTHOR`
  - 관리자 전용:
