...
16. [Content Block API](#16-content-block-api)
...
- 공통 원칙:
  ...
  - `data.type`: canonical `NotificationType` enum (`PARTY_*`, `FRIEND_*`, `*_INVITATION`, `CHAT_MESSAGE`, `NOTICE`, `APP_NOTICE`, `ACADEMIC_SCHEDULE` 등)
...
- 현재 인원이 정원에 도달해도 리더는 모집을 재개할 수 있습니다. 정원이 유지되는 동안 새 공개 동승 요청·친구 초대 발송은 여전히 차단됩니다.
...
- 요청 수락으로 파티 인원이 정원(`maxMembers`)에 도달해도 파티 상태는 자동 전이하지 않습니다.
- 성공 시 파티 채팅방에 서버 생성 `SYSTEM` 메시지 `"{requesterName}님이 입장했어요."`가 추가됩니다. 닉네임이 비어 있거나 찾지 못하면 `"새 멤버가 입장했어요."`를 사용합니다.
- 정원 도달 시 남은 PENDING 동승 요청과 친구 초대만 `EXPIRED + CAPACITY_FULL`로 정리하며, 이후 자리가 생겨도 만료된 항목은 복원하지 않습니다.
...
- 동승 요청 수락이 정원에 도달해도 모집 상태나 `SYSTEM` 모집 마감 메시지는 자동 변경하지 않습니다.
...
- 상세 조회는 호출할 때마다 `viewCount`를 1 증가시킨다. 인증 토큰이 있으면 `isLiked`를 현재 사용자 기준으로 합성하고, 없으면 `false`다.
...
아래 API는 모두 인증이 필요하다. 댓글 응답 구조와 익명·수정·삭제·대댓글·댓글 좋아요 정책은 학교 공지 댓글과 동일하다.

| Method | Path | 설명 |
|---|---|---|
| `POST` | `/v1/app-notices/{appNoticeId}/like` | 앱 공지 좋아요 |
| `DELETE` | `/v1/app-notices/{appNoticeId}/like` | 앱 공지 좋아요 취소 |
| `GET` | `/v1/app-notices/{appNoticeId}/comments` | 댓글/답글 목록 |
| `POST` | `/v1/app-notices/{appNoticeId}/comments` | 댓글/답글 작성 |
| `PATCH` | `/v1/app-notice-comments/{commentId}` | 댓글 수정 |
| `DELETE` | `/v1/app-notice-comments/{commentId}` | 댓글 삭제(soft delete) |
...
- 위 상호작용 API는 인증이 필요하다. 허용 이메일 도메인이 아닌 토큰은 `403 EMAIL_DOMAIN_RESTRICTED`, 탈퇴 회원 토큰은 `403 MEMBER_WITHDRAWN`을 반환한다. 댓글 수정·삭제는 이와 별도로 작성자가 아니면 `403 NOT_APP_NOTICE_COMMENT_AUTHOR`를 반환한다.
...
- 신고 시 `targetType=APP_NOTICE_COMMENT`를 사용한다.
...
- `termsOfUse`는 2026-08-31 공고·시행 기준으로 만 19세 이상 이용, 부적절한 콘텐츠·악성 이용자 무관용, 신고·콘텐츠 차단 및 운영 제재 정책을 포함합니다.
...
| `JOIN_REQUEST_UPDATED` | 동승 요청 상태 변경 (ACCEPTED/DECLINED/CANCELED/EXPIRED) | 해당 파티 리더 |
...
type JoinRequestSsePayload = {
  ...
  invitationInviterName: string | null; // 친구 초대로 생성된 요청의 초대자, 일반 요청이면 null
  status: "PENDING" | "ACCEPTED" | "DECLINED" | "CANCELED" | "EXPIRED";
  expiryReason: "CAPACITY_FULL" | null;
  createdAt: string; // LocalDateTime, "yyyy-MM-dd'T'HH:mm:ss"
};
...
| 파라미터 | 타입 | 설명 |
|---------|------|------|
| `status` | string | 필터 (`PENDING`, `ACCEPTED`, `DECLINED`, `CANCELED`, `EXPIRED`) |
...
| `REOPEN` | 모집 재개 | `CLOSED -> OPEN` (현재 정원과 무관) |
...
- `CANCEL`, `END`는 해당 파티의 PENDING 친구 초대를 `EXPIRED + TARGET_UNAVAILABLE`로 정리한다. 수동 `CLOSE`는 초대를 유지하며, 이후 `REOPEN`해도 이미 만료된 초대는 복원하지 않는다.
...
한쪽이 호출하면 friendship, 양방향 즐겨찾기, 양방향 시간표 공유 예외를 제거한다. 성공은 `204 No Content`이며 친구 끊기 알림은 없다.
...
내 기본 공개 범위와 현재 친구에게 적용되는 친구별 예외를 조회한다. 이미 친구가 아닌 대상의 오래된 예외는 응답에 포함하지 않는다.
...
#### `PATCH /v1/timetables/my/sharing-settings`
...
현재 친구의 예외를 삭제해 기본 범위로 되돌린다. 성공은 `204 No Content`다.

#### `GET /v1/timetables/friends/{friendPublicId}?semester=2026-1`
...
| DELETE | `/v1/party-invitations/{invitationId}` | 발송자가 PENDING 초대 취소 또는 수신자가 EXPIRED 초대 목록 삭제 |
...
outcome은 `SENT | ALREADY_PENDING | ALREADY_MEMBER | NOT_ELIGIBLE`다. 결과는 중복 제거 후 첫 등장 요청 순서를 유지하고 각 수신자를 독립 트랜잭션으로 처리한다. `SENT`와 현재 발송자가 만든 `ALREADY_PENDING`만 invitationId를 제공한다. 친구 관계가 없거나 양방향 차단된 대상은 현재 파티 참여 여부와 무관하게 `NOT_ELIGIBLE`로 마스킹한다. eligible 응답은 `friends`, `alreadyPendingFriends`, `alreadyMemberFriends`를 분리하며, 가득 찬 파티도 `canInvite=false`, `unavailableReason=PARTY_FULL`로 정상 조회된다. 초대는 좌석을 예약하지 않는다. OPEN·CLOSED 상태의 현재 파티 참가자는 정원 여유가 있을 때 초대할 수 있으며, CLOSED는 공개 동승 요청만 막는다. 파티장이 보낸 초대는 수신자 수락 시 `result=JOINED`로 즉시 참가하고 같은 파티의 PENDING 동승 요청을 CANCELED로 정리한다. 일반 참가자가 보낸 초대는 `result=LEADER_APPROVAL_PENDING`, `joinRequestId`로 기존 동승 요청 승인 흐름에 진입하며, 같은 파티의 PENDING 요청이 이미 있으면 이를 재사용한다. 여러 ACCEPTED 초대가 같은 요청을 가리키면 `invitationInviterName`은 `respondedAt`, `id` 오름차순 첫 초대자의 닉네임이며, CLOSED 수락은 그 초대 원본 중 현재 참가자가 하나 이상일 때만 허용한다. ACCEPTED 초대의 수락 재호출은 최초 결과와 `joinRequestId`를 그대로 반환하며 이후 동승 요청 처리 상태를 다시 추론하지 않는다. 수락 시 다른 활성 파티가 있으면 초대를 PENDING으로 유지한 채 `409 ALREADY_IN_PARTY`를 반환한다. ARRIVED·ENDED, 정원 도달, 초대자 이탈·관계 상실은 EXPIRED로 고정한다. 정원이 차면 남은 PENDING 동승 요청과 초대는 `EXPIRED + CAPACITY_FULL`로 종료하되 파티 상태는 자동 변경하지 않는다. EXPIRED 초대는 수신자가 DELETE로 DISMISSED 처리해 목록에서 지울 수 있고 audit 이력은 유지한다.
...
| DELETE | `/v1/chat-room-invitations/{invitationId}` | 발송자가 PENDING 초대 취소 또는 수신자가 EXPIRED 초대 목록 삭제 |
...
batch 형식과 outcome 계약은 택시파티 초대와 같고, 친구 관계가 없거나 양방향 차단된 대상의 현재 방 참여 여부는 `NOT_ELIGIBLE`로 마스킹한다. `UNIVERSITY`, `DEPARTMENT`, `GAME`, 공개 `CUSTOM`만 허용하고 `PARTY`·비공개·1:1 방은 지원하지 않는다. 초대는 생성 후 7일에 만료되며 10분 주기 최대 100건 batch와 목록·mutation의 lazy reconciliation을 사용한다. count는 `expiresAt > now`인 PENDING만 DB에서 직접 집계하고 한 호출에서 최대 100건의 시간 만료만 terminal 저장한다. eligible 조회는 expiresAt이 지난 PENDING을 제외하고, 같은 대상 재발송은 기존 행을 EXPIRED + INVITATION_TIMEOUT으로 먼저 확정한 뒤 새 초대를 생성한다. 기한 뒤 취소도 CANCELED가 아니라 EXPIRED + INVITATION_TIMEOUT으로 확정한다. 수신자의 DELETE도 기한이 지났지만 아직 PENDING인 행을 먼저 EXPIRED + INVITATION_TIMEOUT으로 확정한 뒤 DISMISSED 처리한다. 방 삭제·정원 마감·초대자 이탈·기존 참여·친구 해제·차단·학과 자격 변경은 안전한 expiryReason으로 EXPIRED 처리한다. 직접 참여가 마지막 좌석을 채우면 참여자의 초대는 먼저 ALREADY_JOINED, 나머지 PENDING은 CAPACITY_FULL로 만료한다. 회원 탈퇴의 전체 방 제거에서는 발송·수신 PENDING 초대 대상 방을 먼저 잠근 뒤 해당 초대를 MEMBER_WITHDRAWN으로 만료한다. 학과 변경의 기존 학과방 제거에서는 해당 방의 발송 초대를 INVITER_LEFT, 변경 회원이 받은 학과방 PENDING 초대를 ELIGIBILITY_CHANGED로 즉시 만료한다. 관리자 공개방 삭제는 방 잠금 뒤 해당 방의 PENDING 초대를 TARGET_UNAVAILABLE로 정리한다.

받은 초대 목록은 현재 조치 가능한 `PENDING`과 사유 안내가 필요한 `EXPIRED`만 반환한다. `ACCEPTED`, `DECLINED`, `CANCELED`, `DISMISSED` 이력은 V1 목록에서 제외한다. EXPIRED 수신자는 DELETE로 DISMISSED 처리할 수 있으며 기존 expiryReason·respondedAt audit 값은 보존한다. inviter와 대상 aggregate가 삭제·탈퇴 등으로 안전하게 표시될 수 없거나 inviter가 현재 사용자와 양방향 차단 관계이면 해당 요약은 nullable이다. 학과방 초대 대상은 현재 학과가 다르고 그 방에 참여 중이지 않으면 방 ID·이름·인원 수를 노출하지 않고 `target: null`로 마스킹한다.

초대 OpenAPI는 endpoint에서 실제 반환 가능한 오류만 예시로 제공한다. eligible 조회와 발송의 404는 택시 `PARTY_NOT_FOUND | MEMBER_NOT_FOUND`, 공개방 `CHAT_ROOM_NOT_FOUND | MEMBER_NOT_FOUND`를 함께 노출한다. eligible 조회와 발송의 택시 `PARTY_CLOSED`는 ARRIVED·ENDED처럼 초대 불가인 상태를 뜻하며, 수동 CLOSED는 초대 가능한 상태다. 공개방은 `CHAT_ROOM_FULL | MEMBER_PROFILE_INCOMPLETE`, 택시는 `PARTY_CLOSED | MEMBER_PROFILE_INCOMPLETE`를 409 예시로 노출한다. 수락·거절의 403은 각 `*_RECIPIENT_REQUIRED`, DELETE의 403은 발송자도 만료 초대 수신자도 아닌 경우 각 `*_INVITER_REQUIRED`를 사용한다.
...
| `PARTY_INVITATION_INVITER_REQUIRED` | 403 | 발송자 또는 EXPIRED 초대 수신자 이외의 DELETE |
...
| `CHAT_ROOM_INVITATION_INVITER_REQUIRED` | 403 | 발송자 또는 EXPIRED 초대 수신자 이외의 DELETE |
...
> 변경 이력
> - 2026-08-31: 이용약관 운영 정책 갱신 — 만 19세 이상, UGC 무관용·금지 콘텐츠·신고·콘텐츠 차단·운영 제재와 공고 즉시 시행 예외를 반영
