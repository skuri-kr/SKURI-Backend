...
class ChatRoomSummarySnapshotReaderTest {
    ...
    @Test
    void readCurrent_잠금조회한최신방상태로요약snapshot을만든다() {
        ...
        ChatRoomSummaryEventResponse event = deliveries.getFirst().payload();
        assertEquals("member-1", deliveries.getFirst().memberId());
    ...
    @Test
    void readCurrent_읽음시각이있으면현재메시지수로미읽음을계산한다() {
